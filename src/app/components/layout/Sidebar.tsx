    ...
    <aside className="font-quicksand flex flex-col justify-between py-12 lg:py-16 lg:h-screen lg:sticky lg:top-0">
      <div>
        <div className="flex flex-col gap-4">
          ...
          <Typography
            type="h3"
            className="font-semibold text-lg lg:text-xl text-slate-300/90"
          >
            {applicationString.applicationOwnerRole}
          </Typography>
          <Typography className="text-md text-slate-300/80 mt-2 max-w-sm leading-relaxed">
          ...
          {navItems.map((item, index) => (
            <div key={index}>
              <a
                ...
                className={`inline-block transition-transform transition-colors duration-300 hover:translate-x-2 hover:text-amber-400 ${
        ...
        {sidebarNavItems.map((item, index) => {
          ...
          return (
            <a
              ...
              target={item.target}
              rel={item.rel}
              className="inline-block transition-transform transition-colors duration-300 hover:-translate-y-2 hover:text-amber-400"
            >
              <Icon size={24} />
            </a>
          );
